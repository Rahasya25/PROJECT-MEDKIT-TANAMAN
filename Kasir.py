...
def signup():
    ...
    with open("users.csv", "a", newline="") as file:
        ...
        print("\nSignup berhasil! Silahkan login.")

# Fungsi untuk login
def login():
    print('- Masukkan kode "000" pada inputan apapun untuk keluar dari menu Login -')
    while True:
...
# Fungsi untuk melihat produk bagi admin
def lihat_produk_admin(stok_terkini=None):
    try:
            ...
            # Header tabel
            header = ["Kode", "Nama", "Harga (Rp)", "Stok Tersedia", "Riwayat Stok", "Manfaat"]

            # Membaca data transaksi untuk menghitung stok
            try:
                with open("transaksi.csv", "r") as file1:
                    reader1 = csv.DictReader(file1)
                    trans = list(reader1)
            except FileNotFoundError:
                trans = []  # Jika file transaksi tidak ditemukan, kosongkan list transaksi

            # Data untuk tabel
            data = []
            for row in produk:
                stok_tersedia = int(row["Stok"])  # Ambil stok awal dari file produk
                stok_terjual = sum(
                    int(t["Jumlah"]) for t in trans if t["Kode"] == row["Kode"]
                )  # Hitung stok yang terjual berdasarkan transaksi
                stok_lama = stok_tersedia + stok_terjual  # Hitung sisa stok

                # Jika ada stok terkini yang diberikan, gunakan data tersebut
                if stok_terkini and row["Kode"] in stok_terkini:
                    stok_lama = stok_tersedia + stok_terkini[row["Kode"]]
...
# Fungsi untuk melihat produk bagi user
def lihat_produk_user(stok_terkini=None):
    try:
        with open("produk.csv", "r") as file:
            reader = csv.DictReader(file)
            produk = list(reader)
        ...
        # Header tabel
        header = ["Kode", "Nama", "Harga (Rp)", "Stok Tersedia", "Sisa Stok", "Manfaat"]
...
def tambah_obat():
    print("\n==> TAMBAH PRODUK")
    lihat_produk_admin(stok_terkini=None)
    ...
    if pilihan_tambah == "stok":
        try:
            ...
            with open("produk.csv", "w", newline="") as file:
                ...
                print(f"\nStok untuk produk dengan kode '{kode_tambah_stok}' berhasil ditambahkan!")
        except FileNotFoundError:
    ...
    if pilihan_kurang == "stok":
        ...
        try:
            ...
            with open("produk.csv", "w", newline="") as file:
                ...
                print(f"\nStok untuk produk dengan kode '{kode_kurang_stok}' berhasil dikurangi!")
        except FileNotFoundError:
            print("File produk.csv tidak ditemukan.")
        except ValueError:
            print("Jumlah stok harus berupa angka positif,\
    ...
    if pilihan_kurang == "hapus":
        try:
            ...
            if len(produk_baru) < len(produk):
                ...
                print(f"\nProduk dengan kode '{kode}' berhasil dihapus.")
            else:
...
def transaksi(username):
    ...
    lihat_produk_user(stok_terkini)
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            while True:
                ...
                if kode == "000":
                    print("\nKeluar dari sub-menu Tambah Daftar Pembelian.")
                    ...
                    for item in produk:
                        if item["Kode"] == kode:
                            ...
                            if stok_sisa >= jumlah:
                                ...
                                print(f"\n{item['Nama']} sebanyak {jumlah} berhasil ditambahkan ke daftar pembelian.")
                            else:
...
def lihat_riwayat(stok_terkini=None):
    try:
        with open("transaksi.csv", "r") as file:
            reader = csv.DictReader(file)
            rows = list(reader)  # Mengonversi pembaca ke daftar

            if not rows:  # Jika daftar kosong
        ...
        print("\n==> File transaksi.csv tidak ditemukan.")
    except Exception as e:
        print(f"Terjadi kesalahan: {e}")
